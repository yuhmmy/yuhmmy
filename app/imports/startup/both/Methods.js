...
const updateProfileMethod = 'Restaurant.update';
 ...
 */
Meteor.methods({
  'Restaurant.update'({ email, firstName, lastName, bio, title, picture, interests, projects }) {
    Restaurant.update({ email }, { $set: { email, firstName, lastName, bio, title, picture } });
    Restaurant.remove({ profile: email });
    Restaurant.remove({ profile: email });
...
const addRestaurantMethod = 'Projects.add';
...
export { updateProfileMethod, addRestaurantMethod };
